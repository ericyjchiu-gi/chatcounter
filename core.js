...
(() => {
  ...
  const C = window.ChatCounter = {version:'1.7.0', DAY:86400000, owner:crypto.randomUUID(), listeners:new Set()};
  C.now = () => Date.now();
  ...
  C.read = async scope=>{const key=C.key(scope),v=await C.rpc('get',{keys:[key]});if(v[key]!==undefined&&v[key]?.schema!==3)throw C.error('STORAGE_SCHEMA','Saved state schema is unsupported; it was not overwritten.');return v[key]||C.fresh();};
