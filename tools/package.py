...
names={'manifest.json','README.md','RELEASE_NOTES_1.7.3.md','RELEASE_NOTES_1.7.2.md','RELEASE_NOTES_1.7.1.md','RELEASE_NOTES_1.6_TO_1.7.md'}
for block in manifest['content_scripts']:
 for name in block['js']:
  subprocess.run(['node','--check',str(ROOT/name)],check=True)
  names.add(name)
for name in manifest.get('icons',{}).values(): names.add(name)
for name in ('tests/RESULTS.json','tests/STABILITY_172_RESULTS.json','tests/STABILITY_173_RESULTS.json','tests/browser_regression.py','tests/stability_172.py','tests/stability_173.py'):
