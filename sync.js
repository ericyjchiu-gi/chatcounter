/* Native scanner exclusion, durable checkpoints, explicit outcomes, no background-worker dependency. */
(() => {
  ...
  C.connect=async(force=false)=>{
    ...
    connecting=(async()=>{
      const who=await C.auth(force);await C.migrate(who.scope,who.oldScope);C.session=who;
  ...
  function touch(s,j,activity){s.worker={owner:C.owner,stage:j?.key||'',activity,heartbeatAt:C.now(),expiresAt:C.now()+45000};}
  ...
  async function work(scope,reason,manualKeys=[]){
    await C.rpc('probe');controller=new AbortController();const signal=controller.signal,seen=new Set();let heartbeat;
    try{
      ...
      heartbeat=setInterval(()=>C.write(scope,s=>{if(s.worker?.owner===C.owner){s.worker.heartbeatAt=C.now();s.worker.expiresAt=C.now()+45000;}}).catch(e=>{C.lastError=(e.code||'STORAGE_ERROR')+': '+e.message;controller?.abort();}),10000);
      while(!signal.aborted){
        ...
        C.currentStage=key;C.httpSamples=[];
  ...
  C.control=async action=>{
    ...
    if(action==='rebuild'){
      ...
      const result=await C.lock('chatcounter:history-sync:'+who.oldScope,()=>C.write(who.scope,s=>{
        ...
        s.baseline={startedAt:C.now(),anchor:0,stages:[]};C.beginStages(s);s.baseline.stages.forEach(j=>j.force=true);s.recent=null;s.watermark=0;s.paused=false;s.errors={};s.sourceHealth={};s.governor.stageDelayUntil=0;s.governor.softPauseUntil=0;s.governor.burstCount=0;C.log(s,'rebuild');
      }),true);
