...
(() => {
  ...
  C.source = (kind,id='') => ({kind,id,key:kind+':'+id,offset:0,cursor:'0',status:'pending',attempts:0,nextAt:0,pages:0});
  C.beginStages = s => {
  ...
  function upsert(s,j,header,source){
    ...
    j.tasks[id]={id,u,source,status:!j.force&&C.isIndexed(c,u,j.from)?'unchanged':'pending',cursor:!j.force&&c?.complete&&c.u===u&&c.from>j.from?c.boundaryCursor||null:null,pages:0,attempts:0,nextAt:0,from:j.from,end:j.end,provenFrom:c?.from??null,provenTo:c?.to||0,proven:!!c?.complete};
  }
