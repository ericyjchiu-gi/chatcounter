"""v1.7.1 regression scenarios. Offline Chromium DOM and HTTP/storage fixtures."""
...
async def main():
 ...
 async with async_playwright() as pw:
  ...
  assert s['baseline']['stages'][0]['status']=='complete_with_warnings'
  ...
  await action(p,'start-now');assert (await state(p))['baseline']['stages'][1]['status']=='complete_with_warnings'
  ...
  await p.screenshot(path=str(ROOT/'tests/dashboard-collapsed-171.png'),full_page=True)
  ok('Explicit collapse leaves all 3 coverage cards visible; heartbeat does not reopen the body')
  # Read-only diagnostics export from the actual UI.
  await p.locator('[data-collapse]').click()
  ...
  assert data['version']=='1.7.1' and 'recentLog' in data and 'events' not in data
  ...
  await ctx.close()
  # Real failure: Projects 500, not a conversation error; core can make progress.
  ...
  await p.screenshot(path=str(ROOT/'tests/dashboard-project-error-171.png'),full_page=True)
  # Actual button: not just direct API call.
  first=f.project_calls;await p.locator('[data-action="retry"]').click()
  for _ in range(150):
   observed=await state(p)
   if observed['errors']['24h/source/projects:']['attempts']==2 and not await p.evaluate('!!ChatCounter.running'):break
  ...
  assert (await state(p))['errors']['24h/source/projects:']['attempts']==2
  ...
  await ctx.close()
  # Import exactly the user's seven-attempt, pending-Projects pattern.
  ...
  await ctx.close()
  await browser.close()
 report={'version':'1.7.1','passed':len(checks),'environment':'Offline Chromium DOM + mocked HTTP/storage and simulated Web Locks','orion_device_test':False,'checks':checks}
 (ROOT/'tests/STABILITY_171_RESULTS.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))
