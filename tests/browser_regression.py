...
async def main():
 ...
 async with async_playwright() as pw:
  ...
  await p.wait_for_function("ChatCounter.read(ChatCounter.session.scope).then(s=>s.worker?.activity==='Reading message metadata')")
  ...
  s=await state(p2);assert all(j['status']=='complete' for j in s['baseline']['stages']),s
  ...
  f=Fixture();ctx=await f.ctx(browser);p=await page(ctx,promise=True);await action(p,'start');assert len((await state(p))['events'])==4
 ...
 report={'version':'1.7.0','passed':len(checks),'environment':'Chromium / mocked extension storage + ChatGPT HTTP; simulated Web Locks','orion_device_test':False,'checks':checks}
