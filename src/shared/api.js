...
(() => {
  ...
  const transport=(path,init)=>C.fetchTransport?C.fetchTransport(path,init):fetch(path,init);
  ...
  C.auth = async (force=false) => {
    ...
    authJob=(async()=>{
      const r=await transport('/api/auth/session',{credentials:'include',cache:'no-store',signal:AbortSignal.timeout(15000)});
      if(!r.ok)throw C.error('AUTH_REQUIRED','Session request failed. Refresh your signed-in ChatGPT page.',{status:r.status});
      let j;try{j=await r.json();}catch(_){throw C.error('AUTH_REQUIRED','Sign in to ChatGPT to refresh. Saved index retained.');}
      const t=j.accessToken||j.access_token;if(!t)throw C.error('AUTH_REQUIRED','No signed-in session token was returned.');
  ...
  C.request = async (path,scope,signal,reauthed=false) => {
    ...
    try{
      r=await transport(path,{credentials:'include',cache:'no-store',headers:{accept:'application/json',authorization:'Bearer '+token,...(who.account?{'ChatGPT-Account-Id':who.account}:{})},signal:controller.signal});C.noteRequest(r.status,C.now()-start);
