...
(() => {
  'use strict';
  if (globalThis.ChatCounter) return;
  const C = globalThis.ChatCounter = {version:'2.0.1', DAY:86400000, owner:crypto.randomUUID(), listeners:new Set()};
  C.isWorker=typeof document==='undefined';
  C.now = () => Date.now();
  ...
  if(!C.isWorker)window.addEventListener('message',e=>{if(e.source===window&&e.origin===location.origin&&e.data?.source==='CMM_EXT_V17'&&e.data.event==='changed')C.emit();});
